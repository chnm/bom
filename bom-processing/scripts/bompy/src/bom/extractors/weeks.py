...
class WeekExtractor:
    ...
    def extract_weeks_from_dataframes(self, dataframes: List[tuple[pd.DataFrame, str]]) -> List[WeekRecord]:
        """
    ...
    def _find_week_columns(self, df: pd.DataFrame) -> List[str]:
        ...
        required = ["year"]
        optional = ["week_number", "week", "start_day", "end_day", "start_month", "end_month", "unique_identifier"]
    ...
    def _create_week_record(self, row: pd.Series, source_name: str) -> WeekRecord:
        ...
        week_number = int(row.get("week_number", row.get("week", 1))) if pd.notna(row.get("week_number", row.get("week"))) else 1
        ...
        start_month = str(row["start_month"]) if pd.notna(row.get("start_month")) else None
    ...
    def _is_general_bill(self, unique_identifier: Optional[str], start_month: Optional[str], 
                        end_month: Optional[str], start_day: Optional[int], 
                        end_day: Optional[int]) -> bool:
        """
