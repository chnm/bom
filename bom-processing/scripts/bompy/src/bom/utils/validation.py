...
from ..models import (
    BillOfMortalityRecord, CausesOfDeathRecord, ChristeningRecord,
    ParishRecord, WeekRecord, YearRecord
)
...
class SchemaValidator:
    ...
    def validate_dataframe_for_postgres(df: pd.DataFrame, table_name: str) -> List[str]:
        ...
        if table_name == "bill_of_mortality":
            ...
            if all(col in df.columns for col in key_cols):
                ...
                if duplicates.any():
                    errors.append(f"Found {duplicates.sum()} duplicate records for unique constraint")
            ...
            key_cols = ["christening", "week_number", "start_day", "start_month", "end_day", "end_month", "year"]
            if all(col in df.columns for col in key_cols):
                duplicates = df.duplicated(subset=key_cols)
                if duplicates.any():
                    errors.append(f"Found {duplicates.sum()} duplicate records for unique constraint")
    ...
    @staticmethod
    def log_validation_summary(df: pd.DataFrame, table_name: str, errors: List[str]) -> None:
