...
class FoodstuffsProcessor:
    ...
    def _process_row(self, row: pd.Series, foodstuff_columns: List[str], source: str) -> None:
        """
        ...
        unique_id = self._safe_get_value(row, ['unique_identifier', 'Unique Identifier'])
        ...
        for col in foodstuff_columns:
            ...
            record = FoodstuffsRecord(
                ...
                start_day=int(start_day) if start_day and str(start_day).isdigit() else None,
    ...
    def _safe_get_value(self, row: pd.Series, possible_keys: List[str]) -> Optional[str]:
        """
