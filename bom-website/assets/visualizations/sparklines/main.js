...
document.getElementById("facets").innerHTML = '<div id="loading">Loading data...</div>';
...
async function fetchData() {
  ...
  try {
    ...
    const response = await d3.json('https://data.chnm.org/bom/statistics?type=parish-yearly');
    allData = response;
    return allData;
  } catch (error) {
    console.error("Error fetching data:", error);
    document.getElementById("facets").innerHTML = '<div>Error loading data. Please try again later.</div>';
    return null;
  }
}
...
function renderPage(dataFormat, countType) {
  ...
  document.getElementById("facets").innerHTML = '<div id="loading">Processing data...</div>';
  ...
  setTimeout(() => {
    ...
    if (!processedData[cacheKey]) {
      const tidy = Object.keys(allData[1]).slice(2).flatMap((count) => 
        allData.map(d => ({year: d.year, parish_name: d.parish_name, count, amount: d[count]}))
      );
      processedData[cacheKey] = tidy;
    }
    
    // Create the plot with a loading indicator
    document.getElementById("facets").innerHTML = '<div id="loading">Building visualization...</div>';
...
function optimizeDataset(tidy, format, count) {
  ...
  const filteredData = count === 'plague' 
    ...
    config.marks.push(
      Plot.barY(count === 'plague' ? tidy.filter(d => d.count === "total_plague") : count === "both" ? tidy : tidy.filter(d => d.count === "total_buried"), 
        Plot.normalizeY('extent', {
          ...
          stroke: count === "both" ? "count" : (count === "plague" ? plagueColor : burialColor),
          fill: count === "both" ? "count" : (count === "plague" ? plagueColor : burialColor)
        })
      )
    );
