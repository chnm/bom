...
function joinParishData(geometries, bills) {
  ...
  return {
    ...
    features: geometries.features.map((feature) => {
      ...
      const totalDeaths = parishBills.reduce(
        (sum, bill) => sum + bill.count,
        0,
      );
...
function render(d) {
  ...
  parishShapes()
    .then((parish_shp) => {
      ...
      let filtered_shp = {
        ...
        features: parish_shp.features.filter(
          (d) => d.properties.start_yr < yrRange.slice(-4),
        ),
      };
    ...
    .then((joinedData) => {
      ...
      const districts = L.geoJson(joinedData, {
        //was joinedFiltered //joinedDoubleFiltered
        ...
        style: (d) => {
          return {
            ...
            fillColor:
              cause === "Drowned"
                ? getColorDrowned(d.properties.totalDeaths)
                : cause === "Killed"
                  ? getColorKilled(d.properties.totalDeaths)
                  : getColorSuicide(d.properties.totalDeaths),
          };
        },
      });
      ...
      title.onAdd = function (map) {
        ...
        this._div.innerHTML =
          cause === "Drowned"
            ? "<h2>Deaths by Drowning</h2>" +
              "</n><h3>From " +
              yrRange +
              "</h3>"
            : cause === "Killed"
              ? "<h2>Deaths by Murder</h2>" +
                "</n><h3>From " +
                yrRange +
                "</h3>"
              : "<h2>Deaths by Suicide</h2>" +
                "</n><h3>From " +
                yrRange +
                "</h3>";
        return this._div;
      ...
      legend.update = function (map) {
        ...
        for (var i = 0; i < grades.length; i++) {
          this._div.innerHTML +=
            cause === "Drowned"
              ? '<i style="background:' +
                getColorDrowned(grades[i] + 1) +
                '"></i> ' +
                grades[i] +
                (grades[i + 1] ? "&ndash;" + grades[i + 1] + "<br>" : "+")
              : cause === "Killed"
                ? '<i style="background:' +
                  getColorKilled(grades[i] + 1) +
                  '"></i> ' +
                  grades[i] +
                  (grades[i + 1] ? "&ndash;" + grades[i + 1] + "<br>" : "+")
                : '<i style="background:' +
                  getColorSuicide(grades[i] + 1) +
                  '"></i> ' +
                  grades[i] +
                  (grades[i + 1] ? "&ndash;" + grades[i + 1] + "<br>" : "+");
        }
