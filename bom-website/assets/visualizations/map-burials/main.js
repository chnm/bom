...
function fetchDataAndRender(startYear, endYear, countType) {
  ...
  const url = `http://localhost:8090/bom/bills-geometries?start-year=${startYear}&end-year=${endYear}&bill-type=Weekly&count-type=${countType}`;
  ...
  d3.json(url)
    .then((geojsonData) => {
      ...
      if (geojsonData.metadata && geojsonData.metadata.start_year && geojsonData.metadata.end_year) {
        // Get years from metadata
        for (let year = geojsonData.metadata.start_year; year <= geojsonData.metadata.end_year; year++) {
          years.push(year);
        }
      } else {
    ...
    .catch((error) => {
      ...
      // Show error message
      showError(`Error loading data: ${error.message || 'Unknown error'}.<br>Please try again with different parameters.`);
    });
}
...
function populateDropdowns(years, selectedStartYear = null, selectedEndYear = null) {
...
document.getElementById("reset-button").addEventListener("click", () => {
  ...
  if (startYearSelect.options.length > 0 && endYearSelect.options.length > 0) {
    ...
    const lastYear = endYearSelect.options[endYearSelect.options.length - 1].value;
...
function makeMap() {
  try {
    ...
    if (typeof L === 'undefined') {
      container.innerHTML = '<div style="color: red; padding: 20px; text-align: center;">Map library (Leaflet) not loaded. Please refresh the page.</div>';
      return null;
    }
...
function renderDirectGeoJSON(geojsonData, startYr, endYr, countType) {
  ...
  info.update = function (props) {
    const deathCount = countType === 'Plague' ? (props?.total_plague || 0) : (props?.total_buried || 0);
    this._div.innerHTML =
  ...
  function resetHighlight(e) {
    ...
    layer.setStyle({
      ...
      color: "#000000",
      fillOpacity: 0.8,
      fillColor: getColor(countType === 'Plague' 
