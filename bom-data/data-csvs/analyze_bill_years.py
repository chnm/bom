...
def analyze_year_ranges(bill_list, bill_type):
    ...
    if all_years:
        ...
        files_without_years = [f for f in bill_list if not extract_years_from_csv(f, bill_type.lower())]
...
def write_results_to_file(weekly_bills, general_bills, other_bills):
    ...
    with open(output_file, 'w') as f:
        ...
        # Summary
        f.write("SUMMARY:\n")
        f.write(f"Weekly bills: {len(weekly_bills)} files ({len(weekly_years)} unique years)\n")
        f.write(f"General bills: {len(general_bills)} files ({len(general_years)} unique years)\n")
        f.write(f"Other bills: {len(other_bills)} files ({len(other_years)} unique years)\n")
        f.write(f"Total: {len(weekly_bills) + len(general_bills) + len(other_bills)} files\n")
...
def main():
    ...
    output_file, weekly_years, general_years, other_years = write_results_to_file(weekly_bills, general_bills, other_bills)
    ...
    print(f"General bills: {len(general_bills)} files ({len(general_years)} unique years)")
    if general_years:
    ...
    print("This file contains the complete list of all available years for each bill type.")
