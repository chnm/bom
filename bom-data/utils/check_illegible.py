...
def check_illegible_data():
    ...
    file_counts = defaultdict(
        lambda: {"empty": 0, "illegible": 0, "total": 0, "illegible_columns": 0}
    )
    ...
    for filename in csv_files:
        try:
            with open(filename, "r", encoding="utf-8") as f:
                ...
                illegible_columns = [
                    col for col in reader.fieldnames if col == "is_illegible"
                ]
...
def print_results(file_counts):
    ...
    for filename, counts in sorted(file_counts.items()):
        if counts["illegible"] > 0:
            ...
            print(
                f"{filename}: {counts['illegible']} illegible out of {counts['total']} total records ({counts['illegible_columns']} illegible columns)"
            )
    ...
    for filename, counts in sorted(file_counts.items()):
        if counts["total"] > 0:
            print(
                f"{filename:<50} {counts['illegible']:<10} {counts['empty']:<10} {counts['total']:<10} {counts['illegible_columns']:<5}"
            )
