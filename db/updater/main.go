...
func main() {
	...
	weeksQuery := `
		INSERT INTO bom.week (id, week_id, week_no, start_day, start_month, end_day, end_month, year_id, split_year)
		VALUES (
	...
	for _, weekRecord := range weekRecords {
		...
		// Execute the query
		_, err = conn.Exec(context.Background(), weeksQuery, id, weekID, weekNum, startDay, startMonth, endDay, endMonth, yearID, splitYear)
		if err != nil {
